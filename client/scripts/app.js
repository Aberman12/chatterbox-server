...
app.renderMessage = function(messages){
  ...
  for (let i = 0; i < messages.results.length; i++) {
    if (roomname === null) {
      let $message = new CreateMessage(messages.results[i].username, messages.results[i].text, messages.results[i].roomname);
      $(`<div>
            <div class="username ${$message.username}">${$message.username || 'anonymous'}</div> 
